...
/**
 * Letter A Phonics Lesson Component
 * Embeds Jolly Phonics YouTube video and then presents a quiz
 */
function LetterALesson({ lesson }) {
  ...
  // Quiz questions - words that start or don't start with 'A'
  const questions = [
    { word: 'apple', startsWithA: true },
    { word: 'ball', startsWithA: false },
    { word: 'ant', startsWithA: true },
    { word: 'dog', startsWithA: false },
    { word: 'alligator', startsWithA: true },
    { word: 'cat', startsWithA: false },
    { word: 'astronaut', startsWithA: true },
    { word: 'elephant', startsWithA: false },
    { word: 'ambulance', startsWithA: true },
    { word: 'tiger', startsWithA: false },
  ];
  ...
  const speakQuestion = async (word) => {
    try {
      ...
      // Ask the question
      await speak(`Does ${word} start with the letter A?`, { rate: 0.8 });
  ...
  const handleAnswer = async (answer) => {
    const currentQuestion = questions[currentQuestionIndex];
    const isCorrect = answer === currentQuestion.startsWithA;
    ...
    if (isCorrect) {
      ...
      try {
        await speak('Not quite. Try to listen for the A sound.', { rate: 0.8 });
      } catch (error) {
        console.error('Error speaking:', error);
      }
    }
    ...
    setTimeout(async () => {
      ...
      if (nextIndex < questions.length) {
        ...
        await speakQuestion(questions[nextIndex].word);
      } else {
        // Quiz complete
        completeLesson();
      }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: '36px', color: '#333', textAlign: 'center', marginBottom: '20px' }}>
            🅰️ Learn the Letter A Sound
          </h1>
          <p style={{ fontSize: '20px', color: '#666', textAlign: 'center', marginBottom: '30px' }}>
            Watch this video to learn about the letter A and the sound it makes!
          </p>
          ...
          <div style={{
            ...
            <YouTubeEmbed videoId="gnC02Hr2oSk" />
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: '24px', color: '#666' }}>
          Does this word start with the letter A?
        </p>
      </div>
